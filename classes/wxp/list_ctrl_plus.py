...
class ListCtrlPlus(object):
    '''
    extend wx.LstCtrl
    '''
    prefs = None

    def __init__(self, lsc, view, values=None):
        self.lsc = lsc  # listcontrol
        ...
        if self.prefs:
            lsc_font_size = self.prefs.get_value('general.lsc_font_size')
            if lsc_font_size:
                self.lsc.SetFont(
                    wx.Font(
    ...
    def save_custom_column_width(self):
        ...
        if num_columns > 1:
            widths = [str(self.lsc.GetColumnWidth(i)) for i in range(num_columns)]
