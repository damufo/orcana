...
from .w_result_members import ResultMembers
from classes.wxp.view_plus import ViewPlus
from classes.wxp.messages import Messages


class View(ResultMembers):
    def __init__(self, parent):
        ResultMembers.__init__(self, parent=parent)
        self.SetName('result_members')
        self.parent = parent
        self.parent.load_panel(self)
        self.lsc_candidates.SetName('result_members')
        self.lsc_candidates_plus = self.parent.get_lsc_plus(lsc=self.lsc_candidates, parent=self)

        self.lsc_members.SetName('result_members')
    ...
    def set_values(self, result_members):
        self.lbl_relay.SetLabel(result_members.result.relay.long_name)
