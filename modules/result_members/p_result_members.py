...
from .m_result_members import Model
from .v_result_members import View
from .i_result_members import Interactor


def create(parent, result_members):
    '''
    parent is a presenter
    '''
    return Presenter(
            parent=parent,
            model=Model(result_members=result_members),
...
class Presenter(object):
    ...
    def __init__(self, parent, model, view, interactor):
        ...
        self.view.lsc_members_plus.values = self.model.result_members
        self.view.lsc_members_plus.load(custom_column_widths=True)
        if self.model.result_members.num_members == len(self.model.result_members):
            self.view.btn_new_person.Enable(False)
        self.view.set_values(self.model.result_members)

        # self.model.result_members.load_items_from_dbs()
        # self.view.lsc_caplus.values = self.model.result_members
    ...
    def new_person(self):
        result_members = self.model.result_members
        # result_members_view = {}
        # result_members_view['candidates_pos'] = self.view.lsc_candidates_plus.get_sel_pos_item()
        # result_members_view['members_pos'] = self.view.lsc_members_plus.get_sel_pos_item()
        # result_members_view['result_members'] = self.model.result_members
        # self.model.candidates.config.views['result_members'] = result_members_view
        ...
        person.entity = self.model.result_members.champ.entities.get_entity(self.model.candidates.entity_id)
        person.lock = []
        ...
        if person.person_id:  # foi engadida
            result_members.add_member(person)
            self.view.lsc_members_plus.add_last_item()
            if result_members.num_members == len(result_members):
                    self.view.btn_new_person.Enable(False)
    ...
    def select_candidates(self):
        ...
        if (current_selected + current_candidates) > self.model.result_members.num_members:
    ...
    def move_down(self):
        ...
        if idx is not None:
            if idx == (len(self.model.result_members)-1):
                self.view.msg.warning(_("This is already the last element."))
            else:
                self.model.result_members.move_down(idx)
    ...
    def move_up(self):
        ...
        if idx is not None:
            if idx == 0:
                ...
                self.model.result_members.move_up(idx)
                self.view.lsc_members_plus.update_item(idx)
                self.view.lsc_members_plus.update_item(idx - 1)
                self.view.lsc_members_plus.set_sel_pos_item(idx - 1)
                # print(self.view.lsc_members.GetFocusedItem())
    ...
    def save_candidates(self):
        ...
        if idxs:
            ...
            for person in candidates_selected:
                if len(result_members) >= result_members.num_members:
                    self.view.msg.warning("No more members are allowed in this relay.")
                    break
    ...
    def remove_members(self):
        ...
        if idxs:
            for idx in sorted(idxs, reverse=True):
                person = self.model.result_members[idx].person
                self.model.candidates.append(person)
            self.model.candidates.sort()
            self.view.lsc_candidates_plus.load(custom_column_widths=True)        
            self.model.result_members.delete_items(idxs)
            self.view.lsc_members_plus.load(custom_column_widths=True)
