...
from .m_event_categories_categories_add import Model
from .v_event_categories_categories_add import View
from .i_event_categories_categories_add import Interactor
import copy
...
class Presenter(object):
    ...
    def __init__(self, parent, model, view, interactor):
        ...
        champ_categories = copy.copy(self.model.event_categories.champ.categories)
        event = self.model.event_categories.event
        # Remove current categories from list
        idxs_to_remove = []
        for i in self.model.event_categories:
            champ_categories.remove(i.category)
        # Remove incompatible categories
        gender_id = self.model.event_categories.event.gender_id
        if gender_id == "X" and event.event.ind_rel == "R":
            for i in copy.copy(champ_categories):
                print((i.code, i.gender_id))
                if i.gender_id != "X":
                    champ_categories.remove(i)
        elif self.model.event_categories.event.gender_id != "X":
            for i in copy.copy(champ_categories):
                print((i.code, i.gender_id))
    ...
    def acept(self):       
        if self.view.categories_selected:
            ...
            self.model.event_categories.add_items(
