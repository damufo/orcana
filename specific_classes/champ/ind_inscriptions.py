...
from specific_classes.champ.ind_inscription import IndInscription
# from specific_functions import times
# from specific_functions import files


class IndInscriptions(list):
...
#                     save_action='U'))
    ...
    def load_items_from_dbs(self, event_id=None):
        del self[:]  # borra os elementos que haxa
        if event_id:
            sql = (
                "select ind_inscription_id, pool_length, chrono_type, mark_hundredth, "
                "equated_hundredth, date_time, venue, event_id, person_id "
                "from ind_inscriptions where event_id={} order by equated_hundredth ")
            sql = sql.format(event_id)
        else:
            sql = '''
select ind_inscription_id, pool_length, chrono_type, mark_hundredth, 
equated_hundredth, date_time, venue, event_id, person_id 
from ind_inscriptions i 
order by (select e.pos from events e where e.event_id=i.event_id), equated_hundredth '''
        res = self.config.dbs.exec_sql(sql=sql)
        (IND_INSCRIPTION_ID, pool_length, chrono_type, MARK_HUNDREDTH, 
         EQUATED_HUNDREDTH, DATE_TIME, VENUE, EVENT_ID, PERSON_ID) = range(9)
        for i in res:
            person = self.champ.persons.get_person(i[PERSON_ID])
            event = self.champ.events.get_event(i[EVENT_ID])
            self.append(IndInscription(
                    ind_inscriptions=self,
                    ind_inscription_id=i[IND_INSCRIPTION_ID],
    ...
    def list_values(self):
        ...
        for i in self:
            values.append((
                '{}. {} {} {}'.format(i.event.pos, i.event.code,
                                     i.event.category.gender_id,
                                     i.event.category.code),
                i.person.license,
                i.person.surname,
                i.person.name,
                i.person.gender_id,
